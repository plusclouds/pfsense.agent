...
<?php
/*
 * Lists pfSense NAT port-forward rules straight from config.xml via
 * pfSense's own config subsystem, so the result always matches what the
 * webConfigurator's Firewall > NAT > Port Forward page would show.
...
foreach (config_get_path('nat/rule', []) as $rule) {
	...
	$rules[] = [
		...
		'target_port'      => str_replace(':', '-', $rule['local-port'] ?? ''),
		'description'      => $rule['descr'] ?? '',
		'disabled'         => array_key_exists('disabled', $rule),
	];
}

echo json_encode(['status' => 'ok', 'port_forwards' => $rules]);
