...
<?php
/*
 * Deletes a pfSense NAT port-forward rule matched by the "tracker" id
...
if ($index === null) {
	fwrite(STDERR, "port forward not found: {$tracker}\n");
	exit(1);
}

unset($rules[$index]);
config_set_path('nat/rule', array_values($rules));
write_config("NAT port-forward {$tracker} removed via PlusClouds agent");
filter_configure();
